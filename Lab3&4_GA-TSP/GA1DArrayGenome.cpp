...
GA1DArrayGenome<T>::resize(int len)
{
  ...
  if(len == GAGenome::GENOME_ANY_SIZE)
...
GA1DArrayGenome<T>::resizeBehaviour() const {
  int val = maxX;
  if(maxX == minX) val = GAGenome::GENOME_FIXED_SIZE;
  return val;
}
...
GA1DArrayAlleleGenome<ARRAY_TYPE>::UniformInitializer(GAGenome & c)
{
  ...
  child.resize(GAGenome::GENOME_ANY_SIZE); // let chrom resize if it can
...
OnePointCrossover(const GAGenome& p1, const GAGenome& p2, 
  ...
  if(c1 && c2){
    ...
    if(sis.resizeBehaviour() == GAGenome::GENOME_FIXED_SIZE &&
       bro.resizeBehaviour() == GAGenome::GENOME_FIXED_SIZE){
    ...
    else if(sis.resizeBehaviour() == GAGenome::GENOME_FIXED_SIZE ||
	    bro.resizeBehaviour() == GAGenome::GENOME_FIXED_SIZE){
